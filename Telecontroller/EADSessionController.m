...
#define EAD_INPUT_BUFFER_SIZE 128
    ...
    while ([[_session inputStream] hasBytesAvailable])
    {
        ...
        NSString *str = [[NSString alloc] initWithData:_readData  encoding:kCFStringEncodingUTF8];
        NSLog(@"str = %@",str);
        NSLog(@"byte = %x,%x,%x,%x,%x,%x,%x",((const char *)[_readData bytes])[0],((const char *)[_readData bytes])[1],((const char *)[_readData bytes])[2],((const char *)[_readData bytes])[3],((const char *)[_readData bytes])[4],((const char *)[_readData bytes])[5],((const char *)[_readData bytes])[6]);
    }
